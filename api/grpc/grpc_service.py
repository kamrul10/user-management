        ...
        try:
            ...
            if is_active_db:
                return user_pb2.HealthResponse(
                    health="OK", status=f"{grpc.StatusCode.OK}"
                )
        except Exception as ex:
            print(ex)
            return user_pb2.HealthResponse(
                health="FAILING", status=f"{grpc.StatusCode.INTERNAL}"
            )
    ...
    def CreateUser(self, request, context):
        ...
        if len(existig_user):
            return user_pb2.User(
                id=existig_user[0].id,
                name=existig_user[0].name,
                email=existig_user[0].email,
            )
